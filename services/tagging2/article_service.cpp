...
#define MY_WRITE_LINE(args) \
    ...
    while (stream >> arg) {
        ...
        if ("method" == key) {
            if ("concur" == value)
                ...
                THROW_RUNTIME_ERROR("Invalid method!");
        } else if ("in" == key) {
            infile.swap(value);
        } else if ("out" == key) {
            outfile.swap(value);
        } else if ("k1" == key) {
            if (sscanf(value.c_str(), "%d", &k1) != 1)
                THROW_RUNTIME_ERROR("Invalid k1 value!");
        } else if ("k2" == key) {
            if (sscanf(value.c_str(), "%d", &k2) != 1)
                THROW_RUNTIME_ERROR("Invalid k2 value!");
        } else if ("searchk" == key) {
    ...
    if (method < 0)
        THROW_RUNTIME_ERROR("Please specify method type via method:value");
    if (!k1)
        THROW_RUNTIME_ERROR("Please specify k1 via k1:value");
    if (!k2)
        THROW_RUNTIME_ERROR("Please specify k2 via k2:value");
