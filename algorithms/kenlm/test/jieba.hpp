...
public:
    ...
    struct Gram {
        typedef std::shared_ptr<Gram>  pointer;
        Gram() = default;
        Gram(const std::string &_Word) : word(_Word) {}
        Gram(const std::string &_Word, const std::string &_Type)
                : word(_Word), type(_Type) {}
        std::string word;
        std::string type;
    };

    struct GramPtrCmp {
        bool operator() (const Gram::pointer &lhs, const Gram::pointer &rhs) const
...
public:
    ...
    void wordSegment(const std::string &text, std::vector<Gram> &result)
    {
        TagResult tagResult;
        tagging(text, tagResult);

        result.resize( tagResult.size() );
        for (std::size_t i = 0; i != result.size(); ++i) {
            result[i].word.swap(tagResult[i].first);
            result[i].type.swap(tagResult[i].second);
        } // for
    }
...
inline
std::ostream& operator << (std::ostream &os, const Jieba::Gram &g)
{
    os << g.word << ":" << g.type;
    return os;
}
