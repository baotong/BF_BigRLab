...
{
    std::cerr << "DBG Bigraph::Load() name = " << name << std::endl;
...
{
    if (u.Load(uIdxStream, uLnkStream) && v.Load(vIdxStream, vLnkStream))
        return true;
    return false;
}
...
void Bigraph::Generate( std::ostream &uIdxStream,
                        std::ostream &vIdxStream,
                        std::ostream &uLnkStream,
                        std::ostream &vLnkStream,
                        std::vector<std::pair<TUID, TVID>> &edge_list, TVID nv)
{
    TUID nu = 0;
    for (auto &e : edge_list)
        nu = std::max(nu, e.first);
    ...
    TEID off = 0;  // uint64_t
    uIdxStream.write((char*)&off, sizeof(off));
    for (TUID i = 1; i <= nu; i++)
    {
        while (off < edge_list.size() && edge_list[off].first < i)
        {
            auto tar = edge_list[off].second;
            uLnkStream.write((char*)&tar, sizeof(tar));
            off++;
        }
        uIdxStream.write((char*)&off, sizeof(off));
    }
    ...
    off = 0;
    vIdxStream.write((char*)&off, sizeof(off));
    for (TVID i = 1; i <= nv; i++)
    {
        while (off < edge_list.size() && edge_list[off].second < i)
        {
            auto src = edge_list[off].first;
            vLnkStream.write((char*)&src, sizeof(src));
            off++;
        }
        vIdxStream.write((char*)&off, sizeof(off));
    }
