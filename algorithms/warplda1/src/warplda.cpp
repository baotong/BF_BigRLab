...
template <unsigned MH>
void WarpLDA<MH>::storeZ(std::ostream &os)  {
	shuffle->VisitURemoteDataSequential([&](TUID d, TDegree N, const TVID* lnks, RemoteArray64<TData> &data)
    {
        os << N << ' ';
        for (unsigned i = 0; i < N; i++)
        {
            os << lnks[i] << ':' << data[i].oldk << ' ';
        }
        os << '\n';
    });
    os.flush();
...
template <unsigned MH>
void WarpLDA<MH>::storeZ(std::vector<std::pair<uint32_t, uint32_t>> &result)  {
	shuffle->VisitURemoteDataSequential([&](TUID d, TDegree N, const TVID* lnks, RemoteArray64<TData> &data)
    {
        result.reserve(N + result.size());
        for (unsigned i = 0; i < N; i++)
            result.push_back(std::make_pair(lnks[i], data[i].oldk));
    });
}
