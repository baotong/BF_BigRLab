...
class Client(Iface):
  ...
  def handleRequest(self, request):
    ...
    Parameters:
     - request
    """
    self.send_handleRequest(request)
    return self.recv_handleRequest()

  def send_handleRequest(self, request):
...
class Processor(Iface, TProcessor):
  ...
  def process_segment(self, seqid, iprot, oprot):
    ...
    result = segment_result()
    try:
      result.success = self._handler.segment(args.text)
  ...
  def process_handleRequest(self, seqid, iprot, oprot):
    ...
    result = handleRequest_result()
    try:
      result.success = self._handler.handleRequest(args.request)
      msg_type = TMessageType.REPLY
    except (TTransport.TTransportException, KeyboardInterrupt, SystemExit):
      raise
    except InvalidRequest as err:
...
class segment_result(object):
  ...
  thrift_spec = (
    ...
    (1, TType.STRUCT, 'err', (InvalidRequest, InvalidRequest.thrift_spec), None, ), # 1
  )
